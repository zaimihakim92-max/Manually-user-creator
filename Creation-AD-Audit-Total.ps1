...
$btnCibler.Location = New-Object System.Drawing.Point(445, 16)
$btnCibler.Size     = New-Object System.Drawing.Size(125, 26)
$top.Controls.Add($btnCibler)

$btnRecalc = New-Object System.Windows.Forms.Button
$btnRecalc.Text     = "Recalculer"
$btnRecalc.Location = New-Object System.Drawing.Point(445, 46)
$btnRecalc.Size     = New-Object System.Drawing.Size(60, 26)
$top.Controls.Add($btnRecalc)

$btnClear = New-Object System.Windows.Forms.Button
$btnClear.Text     = "Vider"
$btnClear.Location = New-Object System.Drawing.Point(510, 46)
$btnClear.Size     = New-Object System.Drawing.Size(60, 26)
$top.Controls.Add($btnClear)

$lblStatus = New-Object System.Windows.Forms.Label
$lblStatus.Location = New-Object System.Drawing.Point(445, 78)
$lblStatus.Size     = New-Object System.Drawing.Size(125, 46)
...
Add-Field $tabGen 'Initials'     "Initiales"            1 | Out-Null
Add-Field $tabGen 'Surname'      "Nom (Surname)"        2 | Out-Null
Add-Field $tabGen 'DisplayName'  "Nom affiche"          3 | Out-Null
Add-Field $tabGen 'Description'  "Description"          4 | Out-Null
Add-Field $tabGen 'Office'       "Bureau (Office)"      5 | Out-Null
Add-Field $tabGen 'EmailAddress' "Email"                6 | Out-Null
Add-Field $tabGen 'HomePage'     "Page web"             7 | Out-Null
...
function Update-Computed {
    $fn = $script:tbFirst.Text.Trim()
    $ln = $script:tbLast.Text.Trim()
    $lg = $script:tbLogin.Text.Trim()
    $ou = $script:tbOU.Text.Trim()

    $script:fields['GivenName'].Text         = $fn
    $script:fields['Surname'].Text           = $ln
    $script:fields['DisplayName'].Text       = "$ln $fn"
    $script:fields['SamAccountName'].Text    = $lg
    # User logon name (UPN) au format prenom.nom
    $script:fields['UserPrincipalName'].Text = "$fn.$ln@$($script:MailDomain)"
    $script:fields['EmailAddress'].Text      = "$fn.$ln@$($script:MailDomain)"

    # Initiales = 1ere lettre du prenom + 1ere lettre du nom
    $ini = ""
    if ($fn) { $ini += $fn.Substring(0,1).ToUpper() }
    if ($ln) { $ini += $ln.Substring(0,1).ToUpper() }
    $script:fields['Initials'].Text = $ini

    $dn = "CN=$fn $ln,$ou"
    $script:fields['extensionAttribute12'].Text = "$fn.$ln$($script:DefaultExtAttr12Suffix)"
    $script:fields['extensionAttribute15'].Text = $dn   # provisoire, remplace par le vrai DN a la creation
}
...
$btnCibler.Add_Click({
    ...
    try {
        ...
        # Profil  (les telephones NE SONT PAS repris du modele)
        ...
        $fields['OfficePhone'].Text   = ""
        $fields['MobilePhone'].Text   = ""
...
# ---------- VIDER ----------
$btnClear.Add_Click({
    foreach ($t in @($tbFirst,$tbLast,$tbLogin,$tbModel,$tbOU)) { $t.Clear() }
    foreach ($k in @($fields.Keys)) { $fields[$k].Text = "" }
    $fields['Description'].Text          = $DefaultDescription
    $fields['employeeType'].Text         = $DefaultEmployeeType
    $fields['extensionAttribute13'].Text = $DefaultExtAttr13
    $fields['Password'].Text             = New-RandomPassword
    ...
    $chkClearExch.Checked = $ClearExchangeDefault
    $clbModel.Items.Clear()
    $clbAdd.Items.Clear(); foreach ($g in $DefaultGroupsToAdd)    { $clbAdd.Items.Add($g, $true) | Out-Null }
    $clbRem.Items.Clear(); foreach ($g in $DefaultGroupsToRemove) { $clbRem.Items.Add($g, $true) | Out-Null }
    $Global:SourceUserFull = $null
    $lblStatus.Text      = "Aucun modele charge"
    $lblStatus.ForeColor = [System.Drawing.Color]::Gray
})
...
$btnCreer.Add_Click({
    try {
        ...
        $optMap = @{
            Initials='Initials'; Office='Office'; Company='Company'; Title='Title'; Department='Department'
